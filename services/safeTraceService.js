...
async function getOptimizedRoutes(startLat, startLng, endLat, endLng, profile = 'foot-walking') {
    ...
    // Create avoidance polygons for ONLY Critical zones (not High)
    // Use smaller radius to avoid blocking all routes
    const criticalZones = verifiedZonesPreview.filter(zone => zone.riskLevel === 'Critical');
    
    const avoidAreas = criticalZones
        .map(zone => {
            // Use 50% of the actual radius to create smaller avoidance areas
            const reducedRadius = zone.radius * 0.5;
            const radiusInDegrees = reducedRadius / 111320; // Convert meters to degrees (approximate)
            return [
        ...
        .slice(0, 5); // Limit to 5 most critical zones to prevent blocking all routes

    console.log(`Found ${verifiedZonesPreview.length} verified zones (${criticalZones.length} critical), avoiding ${avoidAreas.length} critical areas`);

    let routes;
    let usedAvoidance = avoidAreas.length > 0;
    
    try {
        // Try with danger zone avoidance first (if any critical zones exist)
        routes = await fetchRoutes(startLat, startLng, endLat, endLng, profile, 3, avoidAreas);
    } catch (error) {
        // If avoidance caused "no route found", try again WITHOUT avoidance
        if (avoidAreas.length > 0 && error.message.includes('No route found')) {
            console.log('Route with avoidance failed, retrying without danger zone avoidance...');
            routes = await fetchRoutes(startLat, startLng, endLat, endLng, profile, 3, []);
            usedAvoidance = false;
        } else {
    ...
    return {
        ...
        metadata: {
            ...
            avoidedAreas: usedAvoidance ? avoidAreas.length : 0,
